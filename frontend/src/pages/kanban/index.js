	...
	const [file, setFile] = useState({
	  ...
	  const popularCards = async () => {
		try {
		  ...
		  const cards = tickets?.filter(ticket => ticket.status === 'open')
		  ?.map(ticket => ({
			...
			draggable: true,
			url: 'https://bipebot.com.br/tickets/' + ticket.id, 
		  }));
		  cards.forEach(card => {
			card.button = `<a href="${card.url}" target="_blank">Abrir Ticket</a>`;
		  });
